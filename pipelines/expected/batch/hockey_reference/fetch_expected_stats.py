...
def fetch_team_stats(team_abbrev: str, season: int) -> list[dict[str, Any]]:
    ...
    return parse_team_page_stats(html)


def parse_team_page_stats(html: str) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        cleaned_cells = [strip_html(cell) for cell in cells]
        if not cleaned_cells:
            continue

        player_name = cleaned_cells[0]
        ...
        games_played = int(cleaned_cells[1].replace(",", "")) if cleaned_cells[1].replace(",", "").isdigit() else 0
        goals = int(cleaned_cells[2].replace(",", "")) if cleaned_cells[2].replace(",", "").isdigit() else 0
        assists = int(cleaned_cells[3].replace(",", "")) if cleaned_cells[3].replace(",", "").isdigit() else 0
        shots = int(cleaned_cells[4].replace(",", "")) if cleaned_cells[4].replace(",", "").isdigit() else 0
...
def build_expected_baseline_for_game(
    ...
    fetch_team_stats_fn: Callable[[str, int], list[dict[str, Any]]] | None = None,
    fetch_team_stats: Callable[[str, int], list[dict[str, Any]]] | None = None,
